...
public interface TaxRateGroupMapper {
	//增加的方法
/*   @Insert("insert into EP_TAXRATE_GROUP values(#{uId},#{groupId},#{groupNo},#{startTime},#{endTime},#{remark},#{createdBy},date_format(now(),'%y-%m-%d'),#{updatedBy},date_format(now(),'%y-%m-%d'),#{isDelete})")
*/    
   ...
   //删除
/*   @Update("update EP_TAXRATE_GROUP set ISDELETE='0' where UID=#{arg0} and GROUP_ID=#{arg1}")
*/    
    void deleteTaxRateGroup(String uId,String groupId);
}                                  
