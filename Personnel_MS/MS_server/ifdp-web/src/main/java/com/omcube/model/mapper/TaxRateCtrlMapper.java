...
public interface TaxRateCtrlMapper {
	...
	/*
	 * @Insert(
	 * "insert into EP_TAXRATE_CTRL values(#{uId},#{groupId},#{applyNo},#{groupLimit},#{groupLowerLimit},#{percentRate},#{quickCal},#{remark},#{createdBy},date_format(now(),'%y-%m-%d'),#{updatedBy},date_format(now(),'%y-%m-%d'),#{isDelete})"
	 * )
	 */
	...
	/*
	 * @Select(
	 * "select UID,GROUP_ID,APPLY_NO,GROUP_LIMIT,GROUP_LOWER_LIMIT,PERCENT_RATE,QUICK_CAL,REMARK,CREATED_BY,CREATED_DATE,UPDATED_BY,UPDATED_DATE,ISDELETE from EP_TAXRATE_CTRL where UID=#{uId}"
	 * )
	 * 
	 * @Results({
	 * 
	...
	/*
	 * @Update(
	 * "update EP_TAXRATE_CTRL set ISDELETE=#{isDelete} where UID=#{arg0} and GROUP_ID=#{arg1} and APPLY_NO=#{arg2}"
	 * )
	 */
