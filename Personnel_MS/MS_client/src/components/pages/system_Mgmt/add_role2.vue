                        ...
                        <el-row :gutter="20">
                            <el-col :span="6" v-for="(funcs, index) in funcsList" :key="index"
                                ...
                                <div class="funcs-content">
                                    <el-checkbox v-model="checkFuncsAll[flag+index]" :indeterminate="isIndeterminate[flag+index]"
                                                 @change="handleFuncsAllChange(checkFuncsAll,funcs,flag,index)"
                                                 class="func-checkall">{{ funcs.menuName }}
                                    </el-checkbox>
                                    <el-checkbox-group v-model="checkFuncs[flag+index]"
                                                       @change="handleCheckedFuncsChange($event,funcs,flag,index)"
                                                       class="func-item">
...
<script type='text/ecmascript-6'>
    ...
    import Vue from "vue";
//    import roleChild from "./role_child.vue";
    export default {
        data() {
            return {
                labelPosition: "right",
            ...
            self.$axios
                ...
                .then(res => {
                    ...
                    res.data.data.forEach(item=>{
                        arr.push(item.menuNo)
                    })
                    for (let i = 0; i < 10; i++) {
                        let flag = arr[i]
                        if(flag) {
                            this.$set(this.isIndeterminate,flag+i,true)
                            this.$set(this.checkFuncsAll,flag+i,false)
                            this.$set(this.checkFuncs, flag+i, [])
                        }
                    }
                })
                .catch(() => {
...
//                this.funcsList = []
            ...
            // 功能权限 多选
            handleFuncsAllChange(val, funcs,flag, index) {
                let arr = []
                funcs.bsns.forEach(item => {
                    arr.push(item.bsnNo)
                })
                this.checkFuncs[flag+index] = val ? arr : []
                this.isIndeterminate[flag+index] = false
            },
            handleCheckedFuncsChange(val, funcs,flag, index) {
                console.log(val, funcs,flag, index)
                if (val.length == funcs.bsns.length) {
                    this.isIndeterminate[flag+index] = false
                    this.checkFuncsAll[flag+index] = true
                } else {
                    this.isIndeterminate[flag+index] = true
                    this.checkFuncsAll[flag+index] = false
                }
